...
results_df.to_csv(os.path.join(OUTPUT_DIR, '2-grams-placebo.csv'), index=False)
results_df[results_df['selected']] \
    .sort_values(by='coef_true', key=lambda s: np.abs(s), ascending=False) \
    .to_csv(os.path.join(OUTPUT_DIR, '2-grams-placebo-all.csv'), index=False)
...
if len(selected_idx) > 0:
    ...
    # 保存图像
    fig_path = os.path.join(OUTPUT_DIR, '2-grams-placebo.png')
