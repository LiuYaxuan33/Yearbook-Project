...
# 提取真实标签
#df['label_true'] = df['gender'].map({'Male': 0, 'Female': 1}) # 将性别转换为二元变量
# df['label_true'] = (df['year']>1914).astype(int) # 将年份转换为二元变量
df['label_true'] = df['is_agriculture'].astype(int)  # 将专业标签转换为二元变量
...
# 清除缓存
for b in range(B):
    os.remove(os.path.join(PLACEBO_CACHE_DIR, f'coef_placebo_{b}.npy'))
# 删除缓存目录
os.rmdir(PLACEBO_CACHE_DIR)
